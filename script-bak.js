document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("mousemove", (event) => {
    ...
    // Calculate the percentage position of the mouse relative to the viewport
    const mouseXPercent = (clientX / innerWidth) * 100;
    const mouseYPercent = (clientY / innerHeight) * 100;

    // Calculate tilt angles for rotation
    const tiltX = (mouseYPercent - 50) / 10; // Vertical tilt
    const tiltY = (50 - mouseXPercent) / 10; // Horizontal tilt

    // Calculate shadow offset
    const shadowX = (mouseXPercent - 50) / 2; // Shadow offset X
    const shadowY = (mouseYPercent - 50) / 2; // Shadow offset Y
